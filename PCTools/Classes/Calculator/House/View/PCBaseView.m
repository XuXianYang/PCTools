...
- (UIView *)alertView {
    if (!_alertView) {
        _alertView = [[UIView alloc]initWithFrame:CGRectMake(0, kScreenH - kTopViewHeight - kDatePicHeight, kScreenW, kTopViewHeight + kDatePicHeight)];
...
- (UIView *)topView {
    if (!_topView) {
        _topView =[[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenW, kTopViewHeight + 0.5)];
...
- (UIButton *)leftBtn {
    if (!_leftBtn) {
        ...
        _leftBtn.layer.borderColor = RGB_HEX(0x0066B1, 1.0).CGColor;
        _leftBtn.layer.borderWidth = 1.0f;
        _leftBtn.layer.masksToBounds = YES;
        _leftBtn.titleLabel.font = [UIFont systemFontOfSize:15.0f];
        [_leftBtn setTitleColor:RGB_HEX(0x0066B1, 1.0) forState:UIControlStateNormal];
...
- (UIButton *)rightBtn {
    if (!_rightBtn) {
        ...
        _rightBtn.frame = CGRectMake(kScreenW - 65, 8, 60, 28);
        _rightBtn.backgroundColor = RGB_HEX(0x0066B1, 1.0);
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        _titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(65, 0, kScreenW - 130, kTopViewHeight)];
        _titleLabel.backgroundColor = [UIColor clearColor];
        _titleLabel.font = [UIFont systemFontOfSize:14.0f];
        _titleLabel.textColor = RGB_HEX(0x0066B1, 1.0);
...
- (UIView *)lineView {
    if (!_lineView) {
        _lineView = [[UIView alloc]initWithFrame:CGRectMake(0, kTopViewHeight, kScreenW, 0.5)];
