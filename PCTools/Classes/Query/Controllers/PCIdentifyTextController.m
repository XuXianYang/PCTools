...
@interface PCIdentifyTextController ()<UINavigationControllerDelegate,UIImagePickerControllerDelegate>
...
@implementation PCIdentifyTextController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view.
    self.view.backgroundColor = UIColorFromRGB(0xFFFFFF);
    self.navigationItem.title = @"文字识别拍照";
    [self setupSubView];
}
...
- (void)setupSubView
{
    [self.saoMiaoImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(PCNaviBarHeight + 99);
    }];
...
- (UIButton *)photoBtn
{
    if (_photoBtn == nil) {
        ...
        [_photoBtn setTitle:@"文字识别拍照" forState:UIControlStateNormal];
...
- (void)getPhotoFromLibrary
{
    ...
    if (@available(iOS 13.0, *)) {
        selectImgAlert.overrideUserInterfaceStyle = UIUserInterfaceStyleLight;
    }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    NSString *url = [NSString stringWithFormat:@"%@/%@",PCBaseUrl,PCTextIdentifyUrl];
    ...
    [manager POST:url parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if ([responseObject isKindOfClass:[NSData class]]){
            ...
            if([response[@"status"] isEqualToString:@"0000"]){
                PCIdentifyResultController *textVC = [[PCIdentifyResultController alloc] init];
