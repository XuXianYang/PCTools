...
@interface EUTIdentifyResultController ()
...
@implementation EUTIdentifyResultController
...
- (void)viewDidLoad {
    ...
    [self eutSetupBottomView];
    [self eutSetupBgScrollView];
}
...
- (void)eutSetupBottomView
{
    ...
    for (int i = 0; i < array.count; i++) {
        ...
        [button addTarget:self action:@selector(eutBottonClick:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)eutSetupBgScrollView
{
    [self.bgScrollView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(EUTNaviBarHeight);
...
// 复制文字、导出图片
- (void)eutBottonClick:(UIButton *)button
{
    if (button.tag == 0) {//复制文字
        ...
        if(!self.showImage){
            [CommonTool toastMessage:@"导出失败!"];
            return;
        }
        
        NSData* imageData =  UIImagePNGRepresentation(self.showImage);
